...
function readExistingData() {
    try {
        if (fs.existsSync('wordv3.json')) {
            let fileContent = fs.readFileSync('wordv3.json', 'utf-8');
...
function readReqData() {
    try {
        if (fs.existsSync('reqv3.json')) {
            let fileContent = fs.readFileSync('reqv3.json', 'utf-8');
            return JSON.parse(fileContent);
        }
    } catch (error) {
        console.error("Error reading file:", error);
...
async function processQuery(current, results) {
    for (let i = 0; i < charactersList.length; i++) {
        ...
        let response = await fetch(`http://35.200.185.69:8000/v3/autocomplete?query=${current + charactersList[i]}`);
...
(async () => {
    ...
    for (let i = storedData.length; i < charactersList.length; i++) {
        ...
        fs.writeFileSync('wordv3.json', JSON.stringify(storedData, null, 2), 'utf-8');
        fs.writeFileSync('reqv3.json', JSON.stringify(requestHistory, null, 2), 'utf-8');
